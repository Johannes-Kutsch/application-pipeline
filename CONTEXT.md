...
**Parser Intake**: Internal parser-thread handoff module that processes one **Position Stub** after `discover()` and applies the complete per-stub path before classify queue or **Pool** admission. Owns **Freshness Gate** (`discover`), **Deduplication** (post-discover), **Domain Pre-Filter**, `Parser.enrich()`, **Deduplication** (post-enrich), **Freshness Gate** (`post_enrich`), **Content Gate**, **Deduplication** counter recording, parser-row **Status Display** updates, Parser Intake skip **Log Artifact** emission, **Pool** admission for `judge_pending` rediscovery, and classify queue forwarding. Its public interface exposes one narrow per-stub call and does not return parser-row metric names, dedup-event tuples, log artifact payloads, **Pool** admission records, or classify handoff data. The **Orchestrator** keeps parser/query lifecycle, parser-dead handling, run state, classify worker topology, and final judge flow. `Parser.enrich()` still runs on the parser thread through the **Parser** interface. _Avoid_: gate bundle, new port, parser adapter.
...
- A listing reaches a **Daily Results File** by passing (in order, ADR-0038/0042/0044): parser thread discovery yields a **Position Stub**; **Parser Intake** handles **Freshness Gate** (discover; drop -> `expired`), **Deduplication** (post-discover; skip/**Pool** re-entry/miss), **Domain Pre-Filter** (title; drop -> `out_of_domain`), `Parser.enrich()`, **Deduplication** (post-enrich; backfilled fields/**Pool** re-entry), **Freshness Gate** (post-enrich), and **Content Gate** (body). **Parser Intake** admits rediscovered matched listings directly to **Pool** or forwards accepted listings to the classify queue. The **LLM Enricher** then applies the raw **Relevance Classifier** call, malformed stashing, post-LLM **Freshness Gate**, Card write, and **Deduplication Store** transition. The **Match Judge** picks **Daily Top-5** from the **Pool**.
