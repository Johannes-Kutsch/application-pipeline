...
**Agent Skills**: Tool-consumed agent workflows seeded by `init` (ADR-0035, ADR-0048, ADR-0050). `init` no longer seeds `application-pipeline/agent-skills/`; workflow bodies are inlined directly into `.claude/skills/<workflow>/SKILL.md` and `.codex/skills/<workflow>/SKILL.md`, while tool-local helper docs are seeded in `<tool-root>/_shared/` and linked via `../_shared/...`. Refresh overwrites package-owned skill files (including `_shared`) and preserves user-added dirs. `application-pipeline/skills/` remains retired legacy path. _Avoid_: subagents, prompts (overloaded).
