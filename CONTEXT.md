...
**Parser Intake**: Internal orchestration module that processes one **Position Stub** after `discover()` and before classify queue or **Pool** admission. Owns per-stub decision order on the parser thread: **Freshness Gate** (`discover`) -> **Deduplication** (post-discover) -> **Domain Pre-Filter** -> `Parser.enrich()` -> **Deduplication** (post-enrich) -> **Freshness Gate** (`post_enrich`) -> **Content Gate**. Returns explicit outcomes such as classify-forwarded, Pool-admitted, dropped, retryable enrich failure, oversized body skip, and transient HTTP skip. The Parser thread keeps parser/query lifecycle, parser-dead handling, and delivery to classify queue/Pool collector. `Parser.enrich()` still runs on the parser thread through the **Parser** interface. _Avoid_: gate bundle, new port, parser adapter.
