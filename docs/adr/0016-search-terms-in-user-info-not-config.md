...
`KEYWORDS`, `NEGATIVE_KEYWORDS` moved from `config.py` into markdown files under `<settings-dir>/user-info/search-terms/` (ADR-0019 splits into per-section files). `SearchTerms` object loaded separately. Missing/empty keywords → `SearchTermsError`.
