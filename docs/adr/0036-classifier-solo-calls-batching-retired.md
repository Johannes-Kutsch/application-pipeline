...
> **AMENDED** by ADR-0041. Output shape changes from `{in_domain, extract: StructuredExtract | None}` to `{in_domain, header: str | None, summary: str | None}` (header/summary non-None iff in-domain). The single-call short-circuit on out-of-domain still applies. The classifier call site is now subsumed into the shared **LLM Enricher**, which also owns body fetch + strip — but the one-call-per-Position cadence, the prompt-cache amortisation across the identical prefix, the **Quota Wall** coordination, and the malformed-response handling (no `.seen.json` write, retry next run, raw output stashed to `failures/malformed/` per ADR-0041) are all preserved.
