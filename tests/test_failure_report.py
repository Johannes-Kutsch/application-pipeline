...
def _write_with_writer_at(
    ...
    with patch("application_pipeline.failure_report.datetime") as mock_dt:
        mock_dt.now.return_value = timestamp
        return writer.write_failure(stage, error)
...
class TestFileCreation:
    def test_returns_path_inside_failures_dir_with_iso_timestamp(
        ...
        path = _write_at(_FIXED_TIME, "parser:test", ValueError("boom"), failures_dir)
    ...
    def test_creates_failures_dir_when_missing(self, failures_dir: Path) -> None:
        ...
        write_failure("stage", ValueError("e"), failures_dir)
        assert failures_dir.is_dir()

    def test_no_tmp_file_left_after_write(self, failures_dir: Path) -> None:
        path = _write_at(_FIXED_TIME, "stage", RuntimeError("e"), failures_dir)
    ...
    def test_path_bound_writer_writes_inside_bound_failures_dir(
        ...
        path = _write_with_writer_at(
            _FIXED_TIME, writer, "parser:test", ValueError("boom")
        )
...
class TestMarkdownBody:
    def test_body_contains_stage(self, failures_dir: Path) -> None:
        path = write_failure("orchestrator:init", RuntimeError("oops"), failures_dir)
    ...
    def test_body_contains_error_class(self, failures_dir: Path) -> None:
        try:
            ...
            path = write_failure("stage", exc, failures_dir)
    ...
    def test_body_contains_traceback(self, failures_dir: Path) -> None:
        try:
            try:
                1 / 0
            except ZeroDivisionError as inner:
                raise RuntimeError("enrichment failed") from inner
        ...
        assert "RuntimeError: enrichment failed" in body
        assert (
            "The above exception was the direct cause of the following exception"
            in body
        )

    def test_body_has_no_error_or_log_tail_section(self, failures_dir: Path) -> None:
        try:
            1 / 0
        ...
        assert "**Last 20 log lines:**" not in body
    ...
    def test_heading_contains_tag_when_discoverable(self, failures_dir: Path) -> None:
        with patch.object(importlib.metadata, "version", return_value="v1.2.3"):
            path = write_failure("stage", RuntimeError("e"), failures_dir)
...
class TestNoClobber:
    def test_two_writes_at_different_timestamps_produce_separate_files(
        ...
        path1 = _write_at(t1, "stage", RuntimeError("e1"), failures_dir)
        path2 = _write_at(t2, "stage", RuntimeError("e2"), failures_dir)
        assert path1 != path2
        assert path1.exists()
        assert path2.exists()
