...
`<settings-dir>/.runtime-data/logs/` (ADR-0028) grouped into layer subdirs: `parser/`, `llm/`, `pipeline/`. Pipeline-owned file types:
...
Production **LLM Extractor** evidence uses **Agent Runtime Logs** under `llm/agent-runtime/classify/` and `llm/agent-runtime/judge/` (ADR-0045). Pipeline-owned LLM transcript JSONL retired.
