...
3. **`<settings-dir>/cv-template/cv_skeleton.tex`** (ADR-0027) — format-by-example for `/write-cv`. Refreshable.

Thirteen slots: `recipient_company/name/street/zip_city`, `opening`, `cover_intro/pivot/fit/closing`, `resume_berufserfahrung/ausbildung/projekte`, `skills_block` (mechanically assembled from **Skill Group** pool per ADR-0020).
