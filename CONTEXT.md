...
**Agent Runtime Invocation**: Production adapter/module behind the **Agent Runtime Invocation Port**. Owns `ruhken-agent-runtime` request construction, provider auth forwarding, pinned service/model/effort/tool policy, RuntimeOutcome-to-invocation-result mapping, and pipeline serialization of **Agent Runtime Logs**. Does not own prompt construction, Agent Output Protocol parsing, extractor error semantics, quota-wall policy, or classifier/judge outcome mapping. _Avoid_: LLM Extractor behavior, prompt parser, usage telemetry contract.
