...
Card: `# **{rank}:** {Header}` + `{Summary}`. Header is three-line LLM-authored block (ADR-0024). `{rank}` is the only Renderer-substituted placeholder. `layout.py`, `Layout` dataclass, placeholder system all retire. `init --refresh` deletes `layout.py`.
